...
export const iotOeeAlertRouter = router({
  getConfigs: protectedProcedure.input(z.object({}).optional()).query(async () => { const db = getDb(); const [rows] = await db.execute('SELECT * FROM iot_oee_alert_config ORDER BY created_at DESC'); return rows; }),
  ...
  deleteConfig: protectedProcedure.input(z.object({ id: z.number() })).mutation(async ({ input }) => { const db = getDb(); await db.execute('DELETE FROM iot_oee_alert_config WHERE id = ?', [input.id]); return { success: true }; }),
  ...
  acknowledgeAlert: protectedProcedure.input(z.object({ alertId: z.number() })).mutation(async ({ input, ctx }) => { const db = getDb(); await db.execute('UPDATE iot_oee_alert_history SET acknowledged = 1, acknowledged_at = NOW(), acknowledged_by = ? WHERE id = ?', [ctx.user.id, input.alertId]); return { success: true }; }),
