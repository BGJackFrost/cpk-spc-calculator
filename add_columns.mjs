...
async function addColumns() {
  ...
  const alterStatements = [
    // production_lines
    `ALTER TABLE production_lines ADD COLUMN IF NOT EXISTS imageUrl VARCHAR(500)`,
    // workstations
    `ALTER TABLE workstations ADD COLUMN IF NOT EXISTS imageUrl VARCHAR(500)`,
    // machines
    `ALTER TABLE machines ADD COLUMN IF NOT EXISTS imageUrl VARCHAR(500)`,
    // process_templates
    `ALTER TABLE process_templates ADD COLUMN IF NOT EXISTS imageUrl VARCHAR(500)`,
    // Create jigs table
